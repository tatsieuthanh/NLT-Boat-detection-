...
class uModBusSerial:
    ...
    def __init__(self, uart_id, baudrate=9600, data_bits=serial.EIGHTBITS, stop_bits=serial.STOPBITS_ONE, PARITY = serial.PARITY_NONE, ctrl_pin=None):
        self._uart = serial.Serial(port=uart_id, baudrate=baudrate, bytesize=data_bits, parity=PARITY, stopbits=stop_bits, timeout=None)
        if ctrl_pin is not None:
            self._ctrlPin = ctrl_pin
            GPIO.setwarnings(False)
            GPIO.setmode(GPIO.BOARD)
            GPIO.setup(self._ctrlPin, GPIO.OUT)
        else:
            self._ctrlPin = None
    ...
    def _uart_read(self):
        ...
        for x in range(1, 40):
            response.extend(self._uart.read_all())
            # variable length function codes may require multiple reads
            if len(response)>=1 and self._exit_read(response):
                break
            time.sleep(0.05)
    ...
    def _send_receive(self, modbus_pdu, slave_addr, count):
        ...
        self._uart.read_all()
        if self._ctrlPin:
            GPIO.output(self._ctrlPin, 1)
        self._uart.write(serial_pdu)

        if self._ctrlPin:
            time.sleep((1 + self.char_time_ms*len(serial_pdu))/1000)
            #time.sleep(1)
            GPIO.output(self._ctrlPin, 0)
    ...
    def _validate_resp_hdr(self, response, slave_addr, function_code, count):
        ...
        if (resp_crc[0] != expected_crc[0]) or (resp_crc[1] != expected_crc[1]):
            print('invalid response CRC')

        if (response[0] != slave_addr):
            print('wrong slave address')

        if (response[1] == (function_code + Const.ERROR_BIAS)):
            print('slave returned exception code: {:d}'.format(response[2]))
